...
def get_data():
    sql_select_query = "select n.full_name, cast(f.face_vector as CHAR(65535) CHARACTER SET utf8) face_vector, f.count from name_data n join face_data f on f.name_id = n.name_id;"
    cursor = mydb.cursor()
    cursor.execute(sql_select_query)
    records = cursor.fetchall()
...
def face_df():
    sql_select_query = "select n.full_name, REPLACE(cast(f.face_vector as CHAR(65535) CHARACTER SET utf8),'\n','') as face_vector from name_data n join face_data f on f.name_id = n.name_id;"
    cursor = mydb.cursor()
    cursor.execute(sql_select_query)
    records = cursor.fetchall()
    df = pd.DataFrame(records)
