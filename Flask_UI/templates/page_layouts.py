...
def basic_video():
    ...
    with tag('div', id='basic_vid'):
        ...
        doc.asis(<button id="stop" type="button" data-state="stop">Stop</button>)
        doc.asis(<div class="progress">)
        doc.asis(<progress id="progress" value="0" min="0">)
        doc.asis(<span id="progress-bar"></span>)
        doc.asis(</progress>)
        doc.asis(</div>)
        doc.asis(<button id="mute" type="button" data-state="mute">Mute/Unmute</button>)
